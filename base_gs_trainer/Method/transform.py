...
def projectToSO3(R: torch.Tensor) -> torch.Tensor:
    '''
...
def rotateSHFeaturesRest(
    ...
    try:
        ...
        raise ImportError(
            '[ERROR][transform::rotateSHFeaturesRest] e3nn is required for SH rotation, '
...
def quatMulBatched(q_world: torch.Tensor, q_gauss: torch.Tensor) -> torch.Tensor:
    '''
...
def extractPureRotation(mat_3x3: torch.Tensor) -> torch.Tensor:
    '''
    用 SVD 将任意 3x3 矩阵投影到最接近的纯旋转 (det=+1) 上, 丢弃所有缩放/错切。
    '''
...
def applySimilarityToGaussians(
    gaussians,
    R_left: torch.Tensor,
    scale: torch.Tensor,
    ...
    '''
    将相似变换 (R_left, scale, translate_row) 原地作用于任意 GaussianModel。
    ...
    高斯形状更新 (对 3DGS 的 3 维 _scaling 与 2DGS 的 2 维 _scaling 均成立):
      - _scaling 存的是 log 域, 物理尺度乘 scale, 等价于对全部分量 _scaling += log(scale)。
        2DGS 只存 2 个切向尺度 (法向轴尺度恒为 1 不存储), 各向同性相似缩放对两个
        切向尺度同样适用, 故全张量广播加法对 2 维与 3 维布局均成立。
    ...
    with torch.no_grad():
        ...
        q_new = quatMulBatched(q_world, q_old)
        gaussians._rotation.data = q_new

        if gaussians._features_rest.numel() > 0:
            gaussians._features_rest.data = rotateSHFeaturesRest(
...
def translateGSGeneric(
    ...
    if gaussians is None:
        print('[ERROR][transform::translateGSGeneric]')
    ...
    applySimilarityToGaussians(gaussians, R_left, scale, translate_row)
    return True
...
def scaleGSGeneric(
    ...
    '''
    if gaussians is None:
        print('[ERROR][transform::scaleGSGeneric]')
        print('\t gaussians is None!')
        return False
    ...
    applySimilarityToGaussians(gaussians, R_left, scale_t, translate_row)
    return True
...
def rotateGSGeneric(
    ...
    '''
    if gaussians is None:
        print('[ERROR][transform::rotateGSGeneric]')
        print('\t gaussians is None!')
        return False

    rotation_right_tensor = toTensor(rotation_right, torch.float32, 'cpu').reshape(3, 3)
    R_right_pure = extractPureRotation(rotation_right_tensor)
...
def transformGSGeneric(
    ...
    '''
    if gaussians is None:
        print('[ERROR][transform::transformGSGeneric]')
        print('\t gaussians is None!')
        return False
    ...
    applySimilarityToGaussians(gaussians, R_left, scale_safe, t_left)
    return True
